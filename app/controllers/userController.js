    ...
    validateEmail(req, res)
        ...
        .then((result) => {
            ...
            nodemailer.mailer({
                ...
                subject: 'welcome to geolocation app',
                message: `hi ${result.firstName} thanks for using geolocation`
            });
...
let login = (req, res) => {
    ...
    findUser(req, res)
        ...
        .then((resolve) => {
            ...
            /* if(resolve.userDetails.isadmin ==false){
                let employee = new trackModel({
            ...
            if (err) {
                ...
                newResetToken.save()
                    .then((resetDetail) => {
                        ...
                        let resetEmail = `<a href='http://api.usermanagement.tk${baseUrl}/resetPassword/${userDetail.email}/${token}'>click here</a>`;
                        nodemailer.mailer({
        ...
        .then((userdetail) => {
            if (userdetail) {
                ...
                if (expire < now) {
                    ...
                    return res.status(401).redirect('http://usermanagement.tk/login');
                }
                else {
                    ...
                    resetTokenModel.updateMany({ $and: [{ email: newEmail }, { token: newToken }] }, { confirmed: true })
                        .then((userdetail) => {
                            if (userdetail.nModified === 1) {
                                ...
                                return res.redirect(`http://usermanagement.tk/resetPassword/${newToken}/${newEmail}`);
                            } else {
                ...
                return res.res.status(404).redirect('http://usermanagement.tk/login');
            }
        })
        .catch((error) => {
            ...
            res.status(400);
            return res.redirect('http://usermanagement.tk/login');
...
let logout = (req, res) => {
    console.log(req.user)
    trackModel.find({ userId: req.user.userId })
        .then((data) => {
            if (check.isEmpty(data)) {
                ...
                // { upsert: true }
                trackModel.updateOne({ userId: req.user.userId }, { loggedOut: Date.now() })
                    .then((result) => {
                        console.log(result);
                        //if updated then delete auth token
                    })
                    .catch((error) => {
                        console.log(error);
                        let apiResponse = response.generate(true, 'some error occured', 500, null);
...
let present = (req, res) => {
    trackModel.find({ userId: req.user.userId })
        .then((data) => {
            if (check.isEmpty(data)) {
                //means user is admin so no need to track his/her attendence
                /*   let attendence = new trackModel({
                ...
                trackModel.updateOne({ userId: req.user.userId }, { loggedOut: Date.now() })
                    .then((data) => {
                        if(data.nModified ==1){
                            console.log(data);
                            let apiResponse = response.generate(false, 'attendence updated', 200, null);
                            res.status(200).send(apiResponse);
                        }
                        else{
                    ...
                    .catch((err) => {
                        console.log(err);
                        let apiResponse = response.generate(true, 'unable to update attendence', 500, null);
                        res.status(500).send(apiResponse);
                    })
            }
        })
